...
export function InstallationRegistration({ userId, installation, onClose, onSuccess }: InstallationRegistrationProps) {
  ...
  const getTimezoneForCountry = (countryCode: string): string => {
    const timezoneMap: Record<string, string> = {
      ...
      'FR': 'Europe/Paris', 'GB': 'Europe/London', 'ES': 'Europe/Madrid',
      'IT': 'Europe/Rome', 'PL': 'Europe/Warsaw', 'PT': 'Europe/Lisbon',
  ...
  useEffect(() => {
    ...
    installationsApi.getInstallationComponentData(installation.id, token)
      .then(setComponentData)
      .catch(console.error);
    installationsApi.getConfigs(installation.id, token)
  ...
  useEffect(() => {
    ...
    locationApi.getCountries(token)
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({ ...prev, timezone: getTimezoneForCountry(formData.country) }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editMode && installation) {
      ...
      if (newInvCount < installation.inverter_count && componentData) {
        const removed = componentData.inverters
          .filter(inv => inv.inverter_number > newInvCount && inv.has_measurements)
          .map(inv => inv.inverter_number);
        if (removed.length > 0 && !window.confirm(`Remove inverters ${removed.join(', ')} and all data? Cannot be undone.`)) return;
      }

      if (formData.has_ev_chargers && newChCount < installation.charger_count && componentData) {
        const removed = componentData.chargers
          .filter(ch => ch.charger_number > newChCount && ch.has_measurements)
          .map(ch => ch.charger_number);
        if (removed.length > 0 && !window.confirm(`Remove chargers ${removed.join(', ')} and all data? Cannot be undone.`)) return;
      }
      ...
      try {
        ...
        setError(err instanceof ApiClientError ? err.detail : 'Failed to update installation');
      } finally {
        setLoading(false);
      }
      return;
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.detail : 'Failed to create installation');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="card shadow-xl max-w-2xl w-full mx-4 p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-text">Name *</label>
              <input
                type="text"
                required
                value={formData.name}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                >
                  <option value="">{loadingCountries ? 'Loading...' : 'Select'}</option>
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-text">State</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-text">City *</label>
                <input
                  type="text"
                  required
                  value={formData.city}
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                ...
                <label key={key} className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData[key as keyof InstallationCreate] as boolean}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium mb-1 text-text">Inverters (1-99)</label>
                <input
                  type="number"
                  min="1"
                  max="99"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-text">Chargers (0-99)</label>
                <input
                  type="number"
                  min="0"
                  max="99"
              ...
              <>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => setConfigs({ ...configs, ACTIVE: e.target.checked ? 'true' : 'false' })}
                    className="accent-accent-1"
                  />
                  <span className="text-sm text-text">Active (RPi uploads data)</span>
                </label>

                <div className="grid grid-cols-2 gap-3">
                  {[
                    { key: 'MAX_SOLAR_POWER', label: 'Max Solar Power (kW)' },
                    { key: 'BATTERY_CAPACITY', label: 'Battery Capacity (kWh)' },
                    { key: 'BATTERY_MAX_CHARGE', label: 'Battery Max Charge (kWh/h)' },
