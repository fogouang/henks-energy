...
export function EnergyFlowDiagram({
  ...
  const solarToBattery =
    batteryPower < 0
      ? Math.min(solarPower - solarToHome, Math.abs(batteryPower))
      : 0;
  const solarToGrid = gridExport;
  const batteryToHome = batteryPower > 0 ? batteryPower : 0;
  const gridToHome = gridImport;
  const [totals, setTotals] = useState({ solar: 0, grid: 0, total: 0, epex_avg: 0 });
      ...
      <div className="flex-1 relative min-h-0">
        ...
        >
          ...
          <g transform="translate(85, 60)">
            <path
              d="M15 0 L28 12 L23 12 L23 24 L7 24 L7 12 L2 12 Z"
              fill="#10b981"
              fillOpacity="0.2"
              stroke="#10b981"
              strokeWidth="1"
          ...
          <g transform="translate(10, 58)">
            ...
            <rect
              x="7"
              y="7"
              width={Math.max(0, (batterySoC / 100) * 22)}
              height="12"
              rx="1"
              fill="#8b5cf6"
              fillOpacity="0.6"
              ...
              return (
                <>
                  <circle
                    ...
                    strokeWidth="1"
                  />
                  {/* Arrow down (green = exporting) or up (red = importing) */}
                  {isExporting ? (
                    <polygon points="15,18 11,12 19,12" fill={color} />
                  ) : (
                    <polygon points="15,8 11,14 19,14" fill={color} />
                  )}
                  <text
                    x="15"
                    y="38"
