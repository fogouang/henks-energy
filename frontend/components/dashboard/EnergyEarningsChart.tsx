...
export function EnergyEarningsChart({ installationId, token, className }: EnergyEarningsChartProps) {
  const [period, setPeriod] = useState<'day' | 'week' | 'month'>('day');
  const [data, setData] = useState<EarningsDataPoint[]>([]);
  const [totals, setTotals] = useState({ solar: 0, grid: 0, total: 0, manual_price: 0.25 });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const json = await installationsApi.getEnergyEarnings(installationId, period, token);
        setData(json.data || []);
        setTotals({
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <span className="text-xs text-text-muted">
            Total: €{totals.total.toFixed(2)} · Price: €{totals.manual_price.toFixed(2)}/kWh
          </span>
        </div>
