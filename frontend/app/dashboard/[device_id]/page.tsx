...
import { installationsApi, edgeDevicesApi, measurementsApi, ApiClientError } from '@/lib/api/client';
...
function DashboardContent() {
  ...
  const [inverters, setInverters] = useState<Array<{
    id: number;
    number: number;
    power: number;
    ratedPower: number;
  ...
  const [evChargers, setEvChargers] = useState<Array<{
  ...
  const [revenueData, setRevenueData] = useState({
    ...
    arbitrage: [] as Array<{ time: string; chargePrice: number; dischargePrice: number; profit: number }>,
    evCharging: [] as Array<{ charger: number; revenue: number; margin: number }>,
    ...
    try {
      if (message.type === 'battery_update') {
        setBatteryData(prev => {
          if (!prev) {
            ...
            return {
              ...
              status: (message.data.power_kw ?? 0) > 0 ? 'discharging' : (message.data.power_kw ?? 0) < 0 ? 'charging' : 'idle',
              eveningReserve: 30,
              minimumReserve: 20,
            };
          }
            ...
            return prev.map(inv =>
              inv.id === inverterId
                ? {
                    ...
                    curtailment: message.data.curtailment_percentage ?? inv.curtailment,
                    status: (message.data.curtailment_percentage ?? 0) > 0 ? 'dimmed' : 'active',
                    curtailmentReason: (message.data.curtailment_percentage ?? 0) > 0 ? 'Curtailment active' : undefined,
                  }
                : inv
            );
          } else {
            // Add new inverter if not found (shouldn't happen normally)
            console.warn(`[Dashboard] inverter_update for unknown inverter_id: ${inverterId}`);
            return prev;
          }
        });
        ...
        setGenerator(prev => {
          if (!prev) {
            return {
              status: (message.data.status as 'off' | 'on' | 'starting' | 'error') ?? 'off',
          ...
          return {
            ...prev,
            ...(message.data.status && { status: message.data.status as 'off' | 'on' | 'starting' | 'error' }),
            ...(message.data.fuel_consumption_lph !== undefined && { fuelConsumption: message.data.fuel_consumption_lph }),
            ...(message.data.charging_power_kw !== undefined && { chargingPower: message.data.charging_power_kw }),
            ...(message.data.auto_start_reason !== undefined && { autoStartReason: message.data.auto_start_reason }),
          };
        });
        ...
        setEvChargers(prev => {
          ...
          if (!chargerNumber) {
            console.warn('[Dashboard] ev_charger_update missing charger_number');
            return prev;
          }
          const existingIndex = prev.findIndex(c => c.chargerNumber === chargerNumber);
          if (existingIndex >= 0) {
            ...
            return prev.map(charger =>
              charger.chargerNumber === chargerNumber
                ? {
                    ...
                    batteryEnergy: message.data.source === 'battery' ? (message.data.energy_kwh ?? charger.batteryEnergy) : charger.batteryEnergy,
                    gridEnergy: message.data.source === 'grid' ? (message.data.energy_kwh ?? charger.gridEnergy) : charger.gridEnergy,
            ...
            return [
              ...prev,
              {
                ...
                batteryEnergy: message.data.source === 'battery' ? (message.data.energy_kwh ?? 0) : 0,
                gridEnergy: message.data.source === 'grid' ? (message.data.energy_kwh ?? 0) : 0,
                tariff: 0.35,
      ...
      console.error('[Dashboard] Error processing WebSocket message:', error, message);
    }
  }, []);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const foundInstallation = installations.find(inst => inst.id === foundInstallationId);
        ...
        if (foundInstallationId) {
          try {
            const data = await installationsApi.getLiveData(foundInstallationId, token);
            ...
            if (data.latest_measurements?.battery) {
              ...
              setBatteryData({
                ...
                status: battery.power_kw > 0 ? 'discharging' : battery.power_kw < 0 ? 'charging' : 'idle',
            ...
            if (data.latest_measurements?.inverters && data.inverters) {
              ...
              const updatedInverters = inverterConfigs.map((invConfig: any) => {
                const measurement = inverterMeasurements.find((m: any) => m.inverter_id === invConfig.id);
                if (measurement) {
                  return {
                    id: invConfig.id,
                    ...
                    status: measurement.curtailment_percentage > 0 ? 'dimmed' : 'active',
                    curtailmentReason: measurement.curtailment_percentage > 0 ? 'Curtailment active' : undefined,
                  };
                }
                return {
                  id: invConfig.id,
            ...
            if (data.latest_measurements?.generator && data.generator) {
              ...
              setGenerator({
                ...
                fuelCost: 1.50, // Can be from config
            ...
            // Update EV chargers
            if (data.ev_chargers) {
              const chargerMeasurements = data.latest_measurements?.ev_chargers || [];
              const chargerConfigs = data.ev_chargers;
              const batterySoC = data.latest_measurements?.battery?.soc_percentage || 50;
              
              const updatedChargers = chargerConfigs.map((chargerConfig: any) => {
                const measurement = chargerMeasurements.find((m: any) => m.charger_id === chargerConfig.id);
                ...
                const totalEnergy = sessionEnergy > 0 ? sessionEnergy : (totalRevenue / tariff);
                ...
                if (measurement) {
                  return {
                    ...
                    isActive: measurement.power_kw > 0 || chargerConfig.session_active,
                    batteryEnergy: batteryEnergy > 0 ? batteryEnergy : (measurement.source === 'battery' ? measurement.energy_kwh : 0),
                    gridEnergy: gridEnergy > 0 ? gridEnergy : (measurement.source === 'grid' ? measurement.energy_kwh : 0),
                    tariff: tariff,
                    revenue: totalRevenue > 0 ? totalRevenue : measurement.revenue_eur,
            ...
            const evChargersLatest = data.latest_measurements?.ev_chargers || [];
            
            // Calculate total solar power
            const totalSolarPower = invertersLatest.reduce((sum: number, inv: any) => sum + (inv.power_kw || 0), 0);
            
            // Calculate self-consumption (solar used directly, not exported)
            const selfConsumption = Math.max(0, totalSolarPower - (meterLatest?.export_kw || 0));
            ...
            const totalConsumption = selfConsumption + (meterLatest?.import_kw || 0);
            const autonomy = totalConsumption > 0 
            ...
            const arbitrageScore = Math.abs(batteryPower) > 0.5 ? Math.min(85, Math.abs(batteryPower) * 20) : 0;
            
            // Calculate EV charging margins
            const evRevenue = evChargersLatest.reduce((sum: number, ev: any) => sum + (ev.revenue_eur || 0), 0);
            ...
            const selfConsumptionData = hours.map((time, i) => ({
              time,
              savings: Math.max(0, dailySavings * (0.5 + Math.sin((i / 24) * Math.PI) * 0.5) * (Math.random() * 0.3 + 0.85)),
            }));
            
            // Feed-in data
            const feedInData = hours.map((time, i) => ({
              time,
              revenue: Math.max(0, (meterLatest?.export_kw || 0) * 0.08 * (0.3 + Math.sin((i / 24) * Math.PI) * 0.7)),
            ...
            const arbitrageData = hours.map((time, i) => ({
              ...
              dischargePrice: 0.25 + Math.sin((i / 12) * Math.PI + Math.PI) * 0.1,
              profit: Math.max(0, (0.25 - 0.15) * Math.abs(batteryPower) * 0.1),
            }));
            
            // EV charging data
            const evChargingDataCalc = evChargersLatest.map((ev: any, i: number) => ({
            ...
            const daysBack = 30;
            const totalPaybackData = Array.from({ length: daysBack }, (_, i) => {
              const date = new Date(now);
              date.setDate(date.getDate() - (daysBack - 1 - i));
              return {
                date: date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }),
                cumulative: dailySavings * (i + 1) + evRevenue * (i + 1) * 0.5,
              };
            });
            ...
            setRevenueData({
              ...
              evCharging: evChargingDataCalc.length > 0 ? evChargingDataCalc : [{ charger: 1, revenue: 0, margin: 0 }],
  ...
  const { status: wsStatus } = useWebSocket(installationId, token, handleWebSocketMessage);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="text-lg mb-2" style={{ color: '#6b7280' }}>{t('common.loading')}</div>
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <div className="text-lg mb-2" style={{ color: '#ef4444' }}>{t('common.error')}</div>
              ...
              {!meterData && (
                <div className="absolute inset-0 flex items-center justify-center bg-surface/80 rounded-lg backdrop-blur-sm z-10">
                  <div className="text-center">
                    ...
                    <span className="text-sm text-text-muted">{t('dashboard.waitingForData')}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="md:col-span-2 flex flex-col gap-3">
              <div className="card p-4 flex-1 relative overflow-hidden">
                ...
                {(() => {
                  ...
                  const savingsNorm = Math.min(100, (financialKPIs.savings.day / 50) * 100); // Max €50
                  ...
                  const evNorm = Math.min(100, (financialKPIs.evChargingMargins / 20) * 100); // Max €20
                  ...
                  const points = [
                    ...
                    { x: 200, y: 100 - ((savingsNorm + autonomyNorm) / 2) * 0.7 - 10 },
                  ];
                  ...
                  return (
                    <div className="absolute inset-0 opacity-25 pointer-events-none">
                      <svg className="w-full h-full" viewBox="0 0 200 100" preserveAspectRatio="none">
                        ...
                        <path d={linePath} fill="none" stroke="#10b981" strokeWidth="2" strokeOpacity="0.8" />
                        {/* Data points */}
                        {points.slice(0, 4).map((p, i) => (
                          <circle key={i} cx={p.x} cy={p.y} r="3" fill="#10b981" fillOpacity="0.6" />
                        ))}
                      </svg>
                    </div>
                  );
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="text-xs mb-1" style={{ color: '#6b7280' }}>{t('financial.savings')}</div>
                        <div className="text-xl font-bold" style={{ color: '#10b981' }}>
                          €{financialKPIs.savings.day.toFixed(2)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs mb-1" style={{ color: '#6b7280' }}>{t('financial.autonomy')}</div>
                        <div className="text-xl font-bold" style={{ color: '#10b981' }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="text-xs mb-1" style={{ color: '#6b7280' }}>{t('financial.arbitrageScore')}</div>
                        <div className="text-xl font-bold" style={{ color: '#3b82f6' }}>
                          {financialKPIs.arbitrageScore.toFixed(1)}%
                        </div>
                      </div>
                      <div>
                        <div className="text-xs mb-1" style={{ color: '#6b7280' }}>{t('financial.evChargingMargins')}</div>
              ...
              {inverters.length > 0 && (
                <div className="card p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex-1">
                    {inverters[Math.min(activeInverterTab, inverters.length - 1)] && (
                      <InverterCard
                        inverterNumber={inverters[Math.min(activeInverterTab, inverters.length - 1)].number}
                        power={inverters[Math.min(activeInverterTab, inverters.length - 1)].power}
                        ratedPower={inverters[Math.min(activeInverterTab, inverters.length - 1)].ratedPower}
                        curtailment={inverters[Math.min(activeInverterTab, inverters.length - 1)].curtailment}
                        status={inverters[Math.min(activeInverterTab, inverters.length - 1)].status}
                        curtailmentReason={inverters[Math.min(activeInverterTab, inverters.length - 1)].curtailmentReason}
                      />
                    )}
                  </div>
                  ...
                  {inverters.length > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-4 pt-3 border-t border-border">
                      <button
                        onClick={() => setActiveInverterTab(Math.max(0, activeInverterTab - 1))}
                      ...
                      <button
                        onClick={() => setActiveInverterTab(Math.min(inverters.length - 1, activeInverterTab + 1))}
