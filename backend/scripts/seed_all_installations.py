...
NUM_DATA_POINTS = 300  # Number of measurement points to generate per installation
...
def generate_battery_soc(prev_soc: float, power: float, capacity: float, dt_hours: float) -> float:
    ...
    energy_change = power * dt_hours
    soc_change = (energy_change / capacity) * 100
    new_soc = prev_soc + soc_change
    return max(20, min(100, new_soc))


async def seed_installation_data(session: AsyncSession, installation_id: int, seed_offset: int = 0):
    """Seed measurement data for a specific installation."""
    ...
    if not installation:
        print(f"  ❌ Installation {installation_id} not found, skipping...")
        return
    
    print(f"\n📊 Seeding data for installation {installation_id}: {installation.name}")
    ...
    if not battery:
        battery = Battery(
            ...
            capacity_kwh=10.0 + random.uniform(-2, 5),  # Vary capacity
            evening_reserve_percentage=30.0,
            minimum_reserve_percentage=20.0,
            soc_percentage=50.0 + random.uniform(-20, 30),
    ...
    if not inverters:
        num_inverters = random.randint(1, 3)
        for i in range(1, num_inverters + 1):
            inverter = Inverter(
                installation_id=installation_id,
                inverter_number=i,
                rated_power_kw=4.0 + random.uniform(0, 3),
                current_power_kw=0.0,
            )
    ...
    if not chargers:
        num_chargers = random.randint(1, 2)
        for i in range(1, num_chargers + 1):
            charger = EVCharger(
                installation_id=installation_id,
                charger_number=i,
                tariff_per_kwh=0.30 + random.uniform(0, 0.15),
        ...
        chargers = list(result.scalars().all())
        print(f"  ✅ Created {len(chargers)} EV chargers")
    
    # Get or create main meter
    result = await session.execute(
    ...
    if not main_meter:
        ...
        await session.commit()
        print(f"  ✅ Created main meter")
    
    # Clear existing measurements
    print(f"  🗑️  Clearing existing measurements...")
    ...
    # Generate measurement data with unique seed per installation
    random.seed(installation_id + seed_offset)  # Different random seed per installation
    
    end_time = datetime.now(timezone.utc)
    start_time = end_time - timedelta(minutes=INTERVAL_MINUTES * NUM_DATA_POINTS)
    
    current_time = start_time
    battery_soc = 40.0 + random.uniform(0, 40)  # Different starting SoC
    cumulative_import_kwh = random.uniform(0, 100)
    cumulative_export_kwh = random.uniform(0, 50)
    ...
    for i in range(NUM_DATA_POINTS):
        ...
        # Vary consumption patterns per installation
        consumption_factor = 1.0 + (installation_id % 5) * 0.2
        if 6 <= hour <= 9 or 17 <= hour <= 22:
            consumption = random.uniform(2.0, 4.0) * consumption_factor
        elif 9 < hour < 17:
            consumption = random.uniform(0.5, 1.5) * consumption_factor
        else:
            consumption = random.uniform(0.3, 0.8) * consumption_factor
        
        # EV charging simulation
        ev_power = 0.0
        ev_chance = 0.08 + (installation_id % 3) * 0.03  # Vary EV usage
        for charger in chargers:
            if random.random() < ev_chance:
                charger_power = random.uniform(3.0, 7.0)
                ev_power += charger_power
                revenue = charger_power * dt_hours * charger.tariff_per_kwh
        ...
        if net_power > 0 and battery_soc < 100:
            battery_power = min(net_power, 5.0)
            battery_soc = generate_battery_soc(battery_soc, battery_power, battery.capacity_kwh, dt_hours)
            grid_export = max(0, net_power - battery_power)
            grid_import = 0.0
        elif net_power < 0 and battery_soc > 20:
            battery_power = max(net_power, -5.0)
            battery_soc = generate_battery_soc(battery_soc, battery_power, battery.capacity_kwh, dt_hours)
        ...
        l1_current = random.uniform(0.3, 2.5) if grid_import > 0 else random.uniform(0.1, 0.8)
        l2_current = random.uniform(0.3, 2.5) if grid_import > 0 else random.uniform(0.1, 0.8)
        l3_current = random.uniform(0.3, 2.5) if grid_import > 0 else random.uniform(0.1, 0.8)
    ...
    # Insert all measurements using raw SQL
    print(f"  💾 Inserting measurements...")
    ...
    for charger in chargers:
        charger_revenue = sum(m.revenue_eur for m in ev_measurements if m.charger_id == charger.id)
        charger_energy = sum(m.energy_kwh for m in ev_measurements if m.charger_id == charger.id)
        charger.total_revenue_eur = charger_revenue
        charger.session_energy_kwh = charger_energy
    
    await session.commit()
    
    print(f"  ✅ Done! Battery: {len(battery_measurements)}, Meter: {len(meter_measurements)}, Inverter: {len(inverter_measurements)}, EV: {len(ev_measurements)}")
    print(f"     Final SoC: {battery_soc:.1f}%, Import: {cumulative_import_kwh:.1f} kWh, Export: {cumulative_export_kwh:.1f} kWh")


async def main():
    """Seed data for all installations that have edge devices."""
    engine = create_async_engine(settings.DATABASE_URL)
    async_session = async_sessionmaker(engine, class_=AsyncSession, expire_on_commit=False)
    
    async with async_session() as session:
        # Find all unique installation IDs that have edge devices
        result = await session.execute(
            select(EdgeDevice.installation_id).distinct()
        )
        installation_ids = [row[0] for row in result.fetchall()]
        
        print(f"🔍 Found {len(installation_ids)} installations with edge devices: {installation_ids}")
        
        for idx, installation_id in enumerate(installation_ids):
            await seed_installation_data(session, installation_id, seed_offset=idx * 1000)
        
        print(f"\n✅ Successfully seeded data for all {len(installation_ids)} installations!")
