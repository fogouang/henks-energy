...
async def get_ev_charger_sessions(
    ...
    """Get EV charger sessions sorted by datetime across all chargers."""
    has_access = await check_installation_access(db, current_user, installation_id)
    if not has_access:
    ...
    # Only sessions from last 7 days
    ...
    for charger in chargers:
        meas_result = await db.execute(
            select(EVChargerMeasurement)
            .where(
                EVChargerMeasurement.charger_id == charger.id,
                EVChargerMeasurement.timestamp >= one_week_ago,
            )
        ...
        for m in measurements:
            if m.power_kw > 0:
                if current_session is None:
                    current_session = {"start": m.timestamp}
                    session_measurements = [m.power_kw]
                else:
                    session_measurements.append(m.power_kw)
            else:
                if current_session is not None:
                    minutes = int((m.timestamp - current_session["start"]).total_seconds() / 60)
                    kwh = round(sum(session_measurements) / 6, 2)
                    if kwh > 0:
                        all_sessions.append({
                            "station": charger.charger_number,
        ...
        # Active session
        if current_session is not None and session_measurements:
            last_ts = measurements[-1].timestamp
            minutes = int((last_ts - current_session["start"]).total_seconds() / 60)
            kwh = round(sum(session_measurements) / 6, 2)
            if kwh > 0:
                all_sessions.append({
                    "station": charger.charger_number,
    ...
    all_sessions.sort(key=lambda x: x["started"], reverse=True)
